    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:sqs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:${var.sqs_queue_name}"
      }
    ]
  })
}
...
# SSM Policy - For Session Manager access (optional but useful)
